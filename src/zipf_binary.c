...
int main(void)
{
  ...
  scanf("%s", file_name);
  int fp = open(file_name,O_WRONLY|O_TRUNC|O_CREAT|O_APPEND,0644);
  /*
  if (fp == NULL)
  {
  ...
  */
  ...
  //10^6 so maximum number of values is 10^9
  int values[1000000];


  //入力データを1000分割して書き込み
  for(int roop;roop<1000;roop++){
    // Generate and output zipf random variables
    for (i=0; i<num_values/1000; i++)
    {
	zipf_rv = zipf(alpha, n);
	values[i]=zipf_rv;
    }
      //fprintf(fp, "%d \n", zipf_rv);
     int ret = write(fp,&values[0],sizeof(int)*num_values/1000);
     if(ret==-1){
